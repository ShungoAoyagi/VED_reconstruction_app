/**
 * Marching Cubes implementation for isosurface extraction from a 3D scalar field.
 *
 * The input data is a flat array of size gridSize^3, ordered as [x,y,z]
 * with z varying fastest: index = x * gridSize * gridSize + y * gridSize + z.
 */
...
function interpolateVertex(
  ...
  if (Math.abs(threshold - v1) < 1e-10) return p1;
  if (Math.abs(threshold - v2) < 1e-10) return p2;
  ...
  for (let x = 0; x < gridSize - 1; x++) {
    for (let y = 0; y < gridSize - 1; y++) {
      for (let z = 0; z < gridSize - 1; z++) {
        ...
        for (const [dx, dy, dz] of CORNER_OFFSETS) {
          const cx = x + dx;
          const cy = y + dy;
          const cz = z + dz;
          cornerValues.push(data[getIndex(cx, cy, cz)]);
          cornerPositions.push([cx - center, cy - center, cz - center]);
        }
